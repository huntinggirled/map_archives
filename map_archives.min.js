...
w={zoom:13,center:new google.maps.LatLng(35.681597,139.766092),disableDefaultUI:!0,mapTypeId:google.maps.MapTypeId.ROADMAP},m=new google.maps.Map(document.getElementById("map_archives"),w),s=new google.maps.Polyline({path:new google.maps.MVCArray,strokeColor:"#FF0000",strokeOpacity:1,strokeWeight:2});s.setMap(m);var x=0,r=new google.maps.InfoWindow,p=new google.maps.Marker,d=new google.maps.MVCArray,t=new google.maps.MVCArray,u={width:f.css("width"),height:f.css("height"),zindex:f.css("z-index")},
l="0-0",g=void 0,v=[];a("body").keydown(function(c){(c=a("#seq_marker").attr("seq")||void 0)&&(37===event.which?f.slideMarker(++c):39===event.which&&f.slideMarker(--c))});a.ajax({url:"<$mt:BlogURL$>archives_jsonp.php",dataType:"jsonp",callback:"callback",timeout:5E3}).done(function(a,b,g){v=a;f.loadMarker()}).fail(function(c,b,g){a("#marker_div").empty()}).always(function(a,b,g){});a.fn.loadMarker=function(){var c=v.items,b=[],d=void 0,f=void 0;if(location.search)for(var e=location.search,e=e.substring(1,
e.length),k=[],k=e.split("&"),e=0;e<k.length;e++){var h=[],h=k[e].split("="),n=h[1];if("q"==h[0]&&""!=n){f=new google.maps.LatLng(n.split(",")[0],n.split(",")[1]);g||(l="3-3");break}}a("#sort1").is(":checked")?c.sort(function(a,c){return a.datetime<c.datetime?1:-1}):c.sort(function(a,c){var b=m.getCenter(),d=b.lat(),e=b.lng(),b=e-a.lng,b=Math.sqrt(Math.pow(d-a.lat,2)+Math.pow(b,2)),e=e-c.lng,d=Math.sqrt(Math.pow(d-c.lat,2)+Math.pow(e,2));return b-d});for(e=k=0;e<c.length;e++)if(h=c[e],""!=h.lat&&
""!=h.lng){var q=h.datetime.split("-")[0]+"-"+h.datetime.split("-")[1];b[q]=+b[q]||0;b[q]++;n='<div id="seq_marker" seq="'+k+'" style="height:80px; width:200px; overflow:auto;">';n+='<div>[ <a href="" onclick="$(this).slideMarker('+(k+1)+');return false;">\u2190</a> ] [ <a href="" onclick="$(this).slideMarker('+(k-1)+');return false;">\u2192</a> ]</div>';n+='<a href="'+h.link+'" target="_blank"><img class="widget-img-thumb" src="'+h.thumbnail+'" height="45" width="45" alt="'+h.title+'" title="'+h.title+
'" /></a>';n+='<a href="'+h.link+'" target="_blank">'+h.title+"</a><br />";n+="</div>";!l||"0-0"!=l&&"4-4"!=l&&"1-1"!=l&&l!=q&&"3-3"!=l||(q=new google.maps.LatLng(h.lat,h.lng),d||(d=q),"3-3"==l?f.lat()==q.lat()&&f.lng()==q.lng()&&(a.createMarker(q,h.title,n,k++),1!=k||g||(a("title").append(": "+h.title),a(".archive-header").append(": "+h.title))):"0-0"==l&&10<=e||"4-4"==l&&100<=e||a.createMarker(q,h.title,n,k++))}m.panTo(d);a(this).focus();if(!g){"3-3"==l&&0<k-1&&(a("title").append(" \u4ed6"+(k-1)+
"\u4ef6"),a(".archive-header").append(" \u4ed6"+(k-1)+"\u4ef6"));var c="",d=0,p;for(p in b)e=p.replace("-0","-"),c+='<option value="'+p+'">'+e.split("-")[0]+"\u5e74"+e.split("-")[1]+"\u6708\uff08"+b[p]+"\uff09</option>",d+=b[p];g="";g+='<select onchange="$(this).markerToggle(this[this.selectedIndex].value);return false;">';f&&(g+='<option value="3-3">\u30ea\u30f3\u30af\u5730\u70b9</option>');g+='<option value="0-0">\u76f4\u8fd110\u4ef6</option>';g+='<option value="4-4">\u76f4\u8fd1100\u4ef6</option>';
...
zIndex:1E4});google.maps.event.addListener(e,"click",function(){r.close();if(p.getPosition()&&p.getPosition().equals(e.getPosition())&&p.getPosition().equals(e.getPosition()))return p=new google.maps.Marker,!1;k.open(m,e);r=k;p=e;a("#marker_div select").blur()});a("#polyline").is(":checked")&&s.getPath().insertAt(x++,c);d.push(e);t.push(k)};a.fn.sortToggle=function(){a("#sort1").is(":checked")?(a("#sort2").prop("checked",!0),a("#search_button").attr("disabled",!1),a("#search_button").removeAttr("disabled")):
(a("#sort1").prop("checked",!0),a("#search_button").attr("disabled",!0));a(this).markerToggle()};a.fn.markerToggle=function(c){c&&(l=c);d.forEach(function(a,c){a.setMap(null)});d.clear();t.clear();s.getPath().clear();"2-2"!=l&&a(this).loadMarker()};a.fn.polylineToggle=function(){if(a("#polyline").is(":checked")){0>=d.getLength()&&a(this).loadMarker();for(var c=0;c<d.getLength();c++){var b=d.getAt(c).getPosition();s.getPath().insertAt(c+1,b)}}else s.getPath().clear()};a.fn.screenToggle=function(){if(a("#screen").is(":checked")){var c=
m.getCenter();f.css("position","fixed");f.css("top","0px");f.css("left","0px");f.css("width","100%");f.css("height","100%")}else c=m.getCenter(),f.css("position","relative"),f.css("width",u.width),f.css("height",u.height);google.maps.event.trigger(m,"resize");m.panTo(c)};a.fn.tabToggle=function(){"0px"==a("#screennavi").css("right")?(a("#screennavi").animate({right:"-"+(a("#screennavi").width()-50)+"px",bottom:"-"+(a("#screennavi").height()-17)+"px"},200),a("#navitab a").text("\u25c6 \u958b\u304f")):
(a("#screennavi").animate({right:"0px",bottom:"0px"},200),a("#navitab a").text("\u25c7 \u9589\u3058\u308b"))};a.fn.slideMarker=function(a){var b=d.getAt(0).getZIndex(),f=d.getAt(d.length-1).getZIndex();a<b?a=f:f<a&&(a=b);for(b=0;b<d.length;b++)if(d.getAt(b).getZIndex()==a){r.close();a=d.getAt(b);b=t.getAt(b);b.open(m,a);m.panTo(a.getPosition());r=b;p=a;break}};a.directMarker=function(a){for(var b=0;b<d.length;b++)if(d.getAt(b).getPosition().lat()==a.lat()&&d.getAt(b).getPosition().lng()==a.lng()){r.close();
a=d.getAt(b);b=t.getAt(b);b.open(m,a);m.panTo(a.getPosition());r=b;p=a;break}};a.fn.getCurrentPosition=function(){a("#current_button").attr("disabled",!0);a("#current_div").append(' <img src="img/indi.gif" alt="\u8aad\u307f\u8fbc\u307f\u4e2d..." width="10" height="10" />');navigator.geolocation?navigator.geolocation.getCurrentPosition(function(c){c=new google.maps.LatLng(c.coords.latitude,c.coords.longitude);m.panTo(c);a("#current_button").attr("disabled",!1);a("#current_button").removeAttr("disabled");
